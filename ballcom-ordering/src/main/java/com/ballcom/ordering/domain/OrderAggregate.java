...
public class OrderAggregate extends AggregateRoot {
    ...
    public static OrderAggregate place(UUID customerId, List<OrderItem> items) {
        ...
        UUID orderId = UUID.randomUUID(); // Genereer hier alvast het ID voor de order

        // Bouw de flexibele payload map (Let op de typo fix: "totalAmount")
        Map<String, Object> payload = Map.of(
            "customerId", customerId.toString(), 
            "items", items, 
            "totalAmount", total.toString() // Als string opslaan voorkomt JSON-conversie gezeur
        );
